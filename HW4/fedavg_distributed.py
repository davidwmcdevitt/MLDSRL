# -*- coding: utf-8 -*-
"""FedAvg_Distributed-Copy1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12kxlm4Z5fm-pQ3o1QRrS1R7jISPmscbJ
"""
...
class Client:
    ...
    def train(self, global_weights):
        ...
        return self.model.state_dict(), len(self.train_loader.dataset), self.running_loss
...
class Server:
    ...
    def fedAvg(self, client_weights):
        ...
        self.total_obs = sum(obs for _, obs, _ in client_weights)
        if self.total_obs == 0:
            return

        self.federatedAvg = None
        for model_weights, obs, _ in client_weights:
...
for round_id in range(num_rounds):
    ...
    for client_id in sample:
        client = Client.remote(train_set[client_id])

        client_weights.append(ray.get(client.train.remote(server.get_global_weights())))
        client_results.append(ray.get(client.evaluate.remote()))

    weighted_averages.append(server.weighted_average(client_results))

    print(f"Communication Round {round_id} Accuracy: {weighted_averages[-1]}")

    weighted_loss.append(server.weighted_average([[loss.detach().numpy(), obs] for _, obs, loss in client_weights]))

    print(f"Communication Round {round_id} Loss: {weighted_loss[-1]}")
...
plt.title('Weighted Validation Accuracies Over Time - Distributed')
...
plt.figure(figsize=(10, 6))
plt.plot(weighted_loss, marker='o')

plt.title('Weighted Training Loss Over Time - Distributed')
plt.xlabel('Communication Rounds')
plt.ylabel('Loss')

plt.savefig('results/fedavg_loss.png', dpi=300, format='png', bbox_inches='tight')
